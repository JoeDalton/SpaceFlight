...
class BombLauncher(Weapon):
    """
    Launches bombs from a ship. The counterpart of
    :class:`~space_flight.weapons.laser_cannon.LaserCannon` for the bomb weapon: it
    spawns a slow :class:`Bomb` dropped along the belly, rate-limited by a reload
    delay. Supply is tracked on the ship (Fighter.bomb_supply); this only spawns
    the projectile.
    """
