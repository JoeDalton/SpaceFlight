...
- :meth:`InputSettingsMenuState._load_file` — static YAML loader
- :meth:`InputSettingsMenuState._make_row_data` — row descriptor builder
- :meth:`InputSettingsMenuState._flush_dead_zones` — entry-widget flush
- :meth:`InputSettingsMenuState._on_confirmed` — binding update callback
"""
...
class TestFormatBinding:
    ...
    def test_joystick_known_axis_returns_axis_prefix(self):
        ...
        label = axis.replace("_", " ").title()
        assert _format_binding("joystick", axis) == f"Axis: {label}"

    def test_joystick_button_returns_button_prefix(self):
        assert _format_binding("joystick", "stick_button_1") == "Button: Stick Button 1"

    def test_forced_type_axis_overrides_heuristic(self):
        # "space" is normally a button; forced_type="axis" must win.
        assert _format_binding("keyboard", "space", forced_type="axis") == "Axis: Space"

    def test_forced_type_button_overrides_heuristic(self):
        # "left_x" is normally a gamepad axis; forced_type="button" must win.
        assert (
            _format_binding("gamepad", "left_x", forced_type="button")
            == "Button: Left X"
        )

    def test_unknown_input_type_falls_back_to_button(self):
        assert _format_binding("unknown_device", "left_x") == "Button: Left X"


# ---------------------------------------------------------------------------
# InputSettingsMenuState._load_file
# ---------------------------------------------------------------------------
...
class TestLoadFile:
    def test_parses_yaml_to_dict(self, tmp_path):
        ...
        assert InputSettingsMenuState._load_file(f) == {"input_type": "keyboard"}

    def test_nested_structure_preserved(self, tmp_path):
        f = tmp_path / "cfg.yaml"
        f.write_text("dead_zones:\n  stick: 0.15\n  throttle: 0.04\n")
        result = InputSettingsMenuState._load_file(f)
...
# ---------------------------------------------------------------------------
# InputSettingsMenuState._make_row_data
# ---------------------------------------------------------------------------


class TestMakeRowData:
    def test_dead_zone_header_always_present(self, state, cfg):
        state._working_config = cfg
        headers = [r["text"] for r in state._make_row_data() if r["kind"] == "header"]
        assert "Dead Zones" in headers

    def test_dead_zone_rows_one_per_key(self, state, cfg):
        state._working_config = cfg
        dz_rows = [r for r in state._make_row_data() if r["kind"] == "deadzone"]
    ...
    def test_global_binding_row_per_action(self, state, cfg):
        state._working_config = cfg
        paths = [r["path"] for r in state._make_row_data() if r["kind"] == "binding"]
        assert ("global", "pause") in paths

    def test_keyboard_bindings_included_for_keyboard_type(self, state, cfg):
        state._working_config = cfg  # input_type = "keyboard"
        paths = [r["path"] for r in state._make_row_data() if r["kind"] == "binding"]
    ...
    def test_deadzone_path_tuple_structure(self, state, cfg):
        state._working_config = cfg
        dz_paths = [
            r["path"] for r in state._make_row_data() if r["kind"] == "deadzone"
        ]
        assert ("dead_zones", "stick") in dz_paths


# ---------------------------------------------------------------------------
# InputSettingsMenuState._flush_dead_zones
# ---------------------------------------------------------------------------


def _mock_entry(text: str):
...
class TestFlushDeadZones:
    def test_float_string_converted_to_float(self, state):
        state._working_config = {"dead_zones": {"stick": 0.15}}
        state._dz_entries = {("dead_zones", "stick"): _mock_entry("0.20")}
        state._flush_dead_zones()
        assert state._working_config["dead_zones"]["stick"] == pytest.approx(0.20)
        assert isinstance(state._working_config["dead_zones"]["stick"], float)

    def test_invalid_string_kept_as_string_when_original_is_float(self, state):
        state._working_config = {"dead_zones": {"stick": 0.15}}
        state._dz_entries = {("dead_zones", "stick"): _mock_entry("not_a_number")}
        state._flush_dead_zones()
        assert state._working_config["dead_zones"]["stick"] == "not_a_number"
    ...
    def test_string_original_kept_as_string(self, state):
        ...
        state._dz_entries = {("dead_zones", "stick"): _mock_entry("0.20")}
        state._flush_dead_zones()
        assert state._working_config["dead_zones"]["stick"] == "0.20"
        assert isinstance(state._working_config["dead_zones"]["stick"], str)

    def test_multiple_entries_all_flushed(self, state):
        state._working_config = {"dead_zones": {"stick": 0.15, "throttle": 0.04}}
        state._dz_entries = {
            ("dead_zones", "stick"): _mock_entry("0.20"),
            ("dead_zones", "throttle"): _mock_entry("0.06"),
        }
        state._flush_dead_zones()
        assert state._working_config["dead_zones"]["stick"] == pytest.approx(0.20)
        assert state._working_config["dead_zones"]["throttle"] == pytest.approx(0.06)
    ...
    def test_no_entries_leaves_config_unchanged(self, state):
        ...
        state._dz_entries = {}
        state._flush_dead_zones()
        assert state._working_config["dead_zones"]["stick"] == pytest.approx(0.15)


# ---------------------------------------------------------------------------
# InputSettingsMenuState._on_confirmed
# ---------------------------------------------------------------------------


class TestOnConfirmed:
    def test_none_value_leaves_config_unchanged(self, state):
        state._active_dialog = MagicMock()
        state._working_config = {"global": {"pause": "escape"}}
        state._binding_labels = {}
        state._on_confirmed(("global", "pause"), None, None)
        assert state._working_config["global"]["pause"] == "escape"
    ...
    def test_active_dialog_cleared_regardless_of_value(self, state):
        ...
        state._binding_labels = {}
        state._on_confirmed(("global", "pause"), None, None)
        assert state._active_dialog is None

    def test_new_value_updates_config(self, state):
        state._active_dialog = MagicMock()
        state._working_config = {"global": {"pause": "escape"}}
        state._binding_labels = {}
        state._on_confirmed(("global", "pause"), "button", "f1")
        assert state._working_config["global"]["pause"] == "f1"
    ...
    def test_deep_nested_path_updated(self, state):
        ...
        state._on_confirmed(path, "button", "f")
        assert state._working_config["contexts"]["flight"]["keyboard"]["fire"] == "f"
    ...
    def test_binding_label_updated_when_path_present(self, state):
        ...
        path = ("contexts", "flight", "keyboard", "fire")
        lbl = MagicMock()
        state._binding_labels = {path: lbl}
        state._on_confirmed(path, "button", "f")
    ...
    def test_binding_label_not_touched_when_path_absent(self, state):
        ...
        state._working_config = {"global": {"pause": "escape"}}
        other_lbl = MagicMock()
        state._binding_labels = {("other", "key"): other_lbl}
        state._on_confirmed(("global", "pause"), "button", "f1")
    ...
    def test_axis_binding_label_shows_axis_prefix(self, state):
        ...
        lbl = MagicMock()
        state._binding_labels = {path: lbl}
        state._on_confirmed(path, "axis", "right_trigger")
