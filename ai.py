...
class AutoPilot():
    ...
    def pilot(self, target_direction: np.ndarray = np.zeros(3), reference_distance_m: float = 0.0):
        """
        ...
        TODO : take into account the speed vector instead of ship axes to account for nicer flight
        ...
        self.throttle_rate = AutoPilot.simple_throttle_controller(angle_to_target_deg=self.angle_to_target_deg, reference_distance_m=reference_distance_m)
        ...
        [self.throttle, self.yaw_rate, self.pitch_rate, self.roll_rate] = low_pass_filter_first_order(
            value = np.array([throttle_command, yaw_rate_command, pitch_rate_command, roll_rate_command]),
            previous = np.array([self.throttle, self.yaw_rate, self.pitch_rate, self.roll_rate]),
            dt=dt, rise_time=self.filter_time, fall_time=self.filter_time,
        )

        return self.throttle, self.yaw_rate, self.pitch_rate, self.roll_rate
    
    @staticmethod
    def simple_throttle_controller(angle_to_target_deg: float, reference_distance_m: float)-> float:
        """
        I want the reference distance to go to zero and the angle to target to go to zero
        First, decrease throttle for too high angles, especially if it's been too long => PI controller
